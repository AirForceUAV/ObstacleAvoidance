...
void CScanView::onDrawSelf(CDCHandle dc)
{
	...
	for (int pos = 0; pos < (int)_scan_data.size(); ++pos) {
		...
		map[_scan_data[pos].angle].distance = _scan_data[pos].dist;
	...
	if ((int)_scan_data.size() > picked_point) {
		...
		//float distPixel = _scan_data[picked_point].dist*distScale;//old
		//float rad = (float)(_scan_data[picked_point].angle*PI / 180.0);//old

		float distPixel = (p.distance + obj.m_size)*distScale;
		float rad = (float)(p.angle*PI / 180.0);
